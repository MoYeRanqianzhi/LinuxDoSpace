...
func Load() (Config, error) {
	cfg := Config{
		App: AppConfig{
			...
			AdminPasswordHashes:   mustParseAdminPasswordHashes(os.Getenv("APP_ADMIN_PASSWORD_HASHES")),
...
func validateAdminConfig(app AppConfig) error {
	...
	if hasAdmins && !hasPassword && !hasPasswordHashes {
		return fmt.Errorf("APP_ADMIN_PASSWORD or APP_ADMIN_PASSWORD_HASHES is required when APP_ADMIN_USERNAMES is configured")
	}

	if !hasAdmins && (hasPassword || hasPasswordHashes) {
		return fmt.Errorf("APP_ADMIN_USERNAMES is required when APP_ADMIN_PASSWORD or APP_ADMIN_PASSWORD_HASHES is configured")
	}

	if hasAdmins && hasPasswordHashes && !hasPassword {
		for _, username := range app.AdminUsernames {
			if _, ok := app.AdminPasswordHashes[strings.ToLower(strings.TrimSpace(username))]; !ok {
				return fmt.Errorf("APP_ADMIN_PASSWORD_HASHES must include %s or APP_ADMIN_PASSWORD must be configured as a compatibility fallback", username)
			}
		}
	}
...
// usernames to bcrypt hashes. Keys are normalized to lowercase usernames so the
// authentication layer can look them up case-insensitively.
func mustParseAdminPasswordHashes(raw string) map[string]string {
	...
	if err := json.Unmarshal([]byte(trimmed), &payload); err != nil {
		panic(fmt.Sprintf("invalid APP_ADMIN_PASSWORD_HASHES: %v", err))
	}

	normalized := make(map[string]string, len(payload))
	for rawUsername, rawHash := range payload {
		username := strings.ToLower(strings.TrimSpace(rawUsername))
		hash := strings.TrimSpace(rawHash)
		if username == "" {
			panic("invalid APP_ADMIN_PASSWORD_HASHES: username keys must not be empty")
		}
		if hash == "" {
			panic(fmt.Sprintf("invalid APP_ADMIN_PASSWORD_HASHES: password hash for %s must not be empty", rawUsername))
		}
