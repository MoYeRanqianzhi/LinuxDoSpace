...
`,
	...
	for _, jobID := range jobIDs {
		job, getErr := getMailDeliveryJobTx(ctx, tx, jobID)
		if getErr != nil {
			return nil, getErr
		}
...
func (s *Store) MarkMailDeliveryJobDelivered(ctx context.Context, input MarkMailDeliveryJobDeliveredInput) (model.MailDeliveryJob, error) {
	...
	job, err := getMailDeliveryJobTx(ctx, tx, input.ID)
	if err != nil {
		return model.MailDeliveryJob{}, err
	}
...
// payload from the current transaction.
func getMailDeliveryJobTx(ctx context.Context, tx *sql.Tx, jobID int64) (model.MailDeliveryJob, error) {
...
func loadMailQueueDefaultDailyLimitTx(ctx context.Context, tx *sql.Tx) (int64, error) {
	row := tx.QueryRowContext(ctx, `
...
// single atomic unit.
func consumeMailQueueCatchAllReservationTx(ctx context.Context, tx *sql.Tx, userID int64, count int64, defaultDailyLimit int64, now time.Time) (model.MailDeliveryReservation, error) {
...
func refundMailQueueCatchAllReservationTx(ctx context.Context, tx *sql.Tx, reservation model.MailDeliveryReservation, now time.Time) error {
