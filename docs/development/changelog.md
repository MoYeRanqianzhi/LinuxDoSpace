...
- Added direct MX-delivery fallback for the built-in SMTP relay so namespace
  mail forwarding can still function when `MAIL_RELAY_FORWARD_HOST` is not yet
  configured with a dedicated upstream provider.
